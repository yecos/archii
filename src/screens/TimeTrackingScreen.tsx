...
<div className="animate-fadeIn space-y-4">
        <div className="flex gap-1 skeuo-well rounded-xl p-1 w-fit overflow-x-auto -mx-1 px-1 scrollbar-none">
          {[{ k: 'Tracker', v: 'tracker' as const }, { k: 'Registros', v: 'entries' as const }, { k: 'Resumen', v: 'summary' as const }].map(tab => (
            <button key={tab.v} className={`px-3 py-1.5 rounded-md text-[13px] cursor-pointer transition-all ${tt.timeTab === tab.v ? 'bg-[var(--skeuo-raised)] text-[var(--foreground)] font-medium shadow-[var(--skeuo-shadow-raised-sm)]' : 'text-[var(--muted-foreground)] hover:text-[var(--foreground)]'}`} onClick={() => tt.setTimeTab(tab.v)}>{tab.k}</button>
          ))}
        </div>
        ...
        {tt.timeTab === 'tracker' && (<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="card-elevated rounded-xl p-5">
            ...
            <div className={`skeuo-well rounded-xl text-center py-8 mb-4 ${tt.timeSession.isRunning ? 'border-[var(--af-green)]/30' : ''}`}>
              <div className={`text-5xl font-mono font-bold tracking-wider ${tt.timeSession.isRunning ? 'text-[var(--af-green)]' : 'text-[var(--muted-foreground)]'}`}>{tt.timeSession.isRunning ? fmtTimer(tt.timeTimerMs) : '00:00'}</div>
              <div className="text-xs text-[var(--muted-foreground)] mt-2">{tt.timeSession.isRunning ? 'En curso...' : 'Detenido'}</div>
              {tt.timeSession.isRunning && <div className="w-3 h-3 bg-[var(--af-green)] rounded-full mx-auto mt-2 animate-pulse" />}
            </div>
            <div className="space-y-3 mb-4">
              <select className="w-full skeuo-input px-3 py-2 text-sm text-[var(--foreground)] outline-none" value={ui.forms.teProject || ''} onChange={e => ui.setForms(p => ({ ...p, teProject: e.target.value }))}>
                <option value="">Seleccionar proyecto</option>
                {fs.projects.map(p => <option key={p.id} value={p.id}>{p.data.name}</option>)}
              </select>
              <select className="w-full skeuo-input px-3 py-2 text-sm text-[var(--foreground)] outline-none" value={ui.forms.tePhase || ''} onChange={e => ui.setForms(p => ({ ...p, tePhase: e.target.value }))}>
                <option value="">Fase (opcional)</option>
                {DEFAULT_PHASES.map(ph => <option key={ph} value={ph}>{ph}</option>)}
              </select>
              <input type="text" className="w-full skeuo-input px-3 py-2 text-sm text-[var(--foreground)] outline-none" placeholder="Que vas a hacer?" value={ui.forms.teQuickDesc || ''} onChange={e => ui.setForms(p => ({ ...p, teQuickDesc: e.target.value }))} disabled={tt.timeSession.isRunning} />
              <div className="flex items-center gap-2">
                <span className="text-xs text-[var(--muted-foreground)]">Tarifa/h:</span>
                <input type="number" className="flex-1 skeuo-input px-3 py-2 text-sm text-[var(--foreground)] outline-none" value={ui.forms.teRate || 50000} onChange={e => ui.setForms(p => ({ ...p, teRate: e.target.value }))} />
              </div>
            </div>
            <div className="flex gap-2">
              {!tt.timeSession.isRunning ? (
                <button className="skeuo-btn flex-1 bg-[var(--af-green)] text-white px-4 py-2.5 rounded-lg text-sm font-semibold cursor-pointer border-none hover:opacity-90 transition-opacity" onClick={tt.startTimeTracking}>Iniciar</button>
              ) : (
                <button className="skeuo-btn flex-1 bg-[var(--af-red)] text-white px-4 py-2.5 rounded-lg text-sm font-semibold cursor-pointer border-none hover:opacity-90 transition-opacity" onClick={tt.stopTimeTracking}>Detener</button>
              )}
              <button className="skeuo-btn px-4 py-2.5 rounded-lg text-sm cursor-pointer bg-[var(--skeuo-raised)] text-[var(--foreground)] hover:opacity-80" onClick={() => { ui.setForms(p => ({ ...p, teDescription: '', teProject: '', tePhase: '', teDate: new Date().toISOString().split('T')[0], teStartTime: '', teEndTime: '', teManualDuration: '', teBillable: true, teRate: 50000 })); ui.openModal('timeEntry'); }}>+ Manual</button>
            </div>
          </div>
              ...
              return (
                <div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                    <div className="skeuo-well rounded-xl p-3 text-center"><div className="text-xl font-bold text-[var(--af-accent)]">{fmtDuration(totalToday)}</div><div className="text-[11px] text-[var(--muted-foreground)]">Total hoy</div></div>
                    <div className="skeuo-well rounded-xl p-3 text-center"><div className="text-xl font-bold text-[var(--af-green)]">{fmtDuration(billableToday)}</div><div className="text-[11px] text-[var(--muted-foreground)]">Facturable</div></div>
                  </div>
                  {todayEntries.length === 0 ? <div className="text-center py-8 text-[var(--af-text3)] text-sm">Sin registros hoy</div> : (
                    <div className="space-y-2 max-h-[300px] overflow-y-auto">
                      {todayEntries.map(e => {
                        ...
                        return (
                          <div key={e.id} className="skeuo-well rounded-xl flex items-center gap-3 p-2.5">
                            ...
                            {e.data.billable && <div className="w-2 h-2 rounded-full bg-[var(--af-green)] shrink-0" />}
                          </div>
                        );
                      })}
                    </div>
        ...
        {tt.timeTab === 'entries' && (<div>
          <div className="flex items-center gap-3 mb-4 flex-wrap">
            <select className="skeuo-input px-3 py-2 text-sm text-[var(--foreground)] outline-none" value={tt.timeFilterProject} onChange={e => tt.setTimeFilterProject(e.target.value)}>
              <option value="all">Todos los proyectos</option>
              {fs.projects.map(p => <option key={p.id} value={p.id}>{p.data.name}</option>)}
            </select>
            <input type="date" className="skeuo-input px-3 py-2 text-sm text-[var(--foreground)] outline-none" value={tt.timeFilterDate} onChange={e => tt.setTimeFilterDate(e.target.value)} />
            <button className="skeuo-btn ml-auto flex items-center gap-1.5 bg-[var(--af-accent)] text-background px-3 py-2 rounded-lg text-xs font-semibold cursor-pointer border-none" onClick={() => { ui.setForms(p => ({ ...p, teDescription: '', teProject: '', tePhase: '', teDate: new Date().toISOString().split('T')[0], teStartTime: '', teEndTime: '', teManualDuration: '', teBillable: true, teRate: 50000 })); ui.openModal('timeEntry'); }}>+ Registro manual</button>
          </div>
          {(() => {
                ...
                <div className="card-elevated rounded-xl overflow-hidden">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {filtered.map(e => {
                          const proj = fs.projects.find(p => p.id === e.data.projectId);
                          return (<tr key={e.id} className="border-b border-[var(--skeuo-edge-dark)] last:border-0 hover:bg-[var(--skeuo-raised)] transition-colors">
                            ...
                            <td className="px-4 py-2.5"><button className="text-xs text-[var(--af-red)] cursor-pointer hover:opacity-70" onClick={() => fbActions.deleteTimeEntry(e.id, ui.showToast)}>🗑</button></td>
                          </tr>);
                        })}
                      </tbody>
                    </table>
            ...
            return (<>
              <div className="card-elevated rounded-xl p-5">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="skeuo-well rounded-xl p-3 text-center"><div className="text-2xl font-bold text-[var(--af-accent)]">{fmtDuration(weekTotal)}</div><div className="text-[11px] text-[var(--muted-foreground)]">Horas totales</div></div>
                  <div className="skeuo-well rounded-xl p-3 text-center"><div className="text-2xl font-bold text-[var(--af-green)]">{fmtCOP(weekBillable)}</div><div className="text-[11px] text-[var(--muted-foreground)]">Facturable</div></div>
                </div>
              </div>
              <div className="card-elevated rounded-xl p-5">
                <h3 className="text-[15px] font-semibold mb-4">Este Mes</h3>
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="skeuo-well rounded-xl p-3 text-center"><div className="text-2xl font-bold text-[var(--af-accent)]">{fmtDuration(monthTotal)}</div><div className="text-[11px] text-[var(--muted-foreground)]">Horas totales</div></div>
                  <div className="skeuo-well rounded-xl p-3 text-center"><div className="text-2xl font-bold text-[var(--af-green)]">{fmtCOP(monthBillable)}</div><div className="text-[11px] text-[var(--muted-foreground)]">Facturable</div></div>
                </div>
              </div>
              <div className="card-elevated rounded-xl p-5">
                  ...
                  <div className="space-y-2">
                    {Object.entries(byProject).sort((a, b) => b[1] - a[1]).map(([pid, mins]) => {
                      ...
                      return (<div key={pid}>
                        ...
                        <div className="w-full bg-[var(--skeuo-inset)] shadow-[var(--skeuo-shadow-inset-sm)] rounded-full h-1.5"><div className="bg-[var(--af-accent)] rounded-full h-1.5" style={{ width: `${(mins / Math.max(...Object.values(byProject))) * 100}%` }} /></div>
                      </div>);
                    })}
                  </div>
                )}
