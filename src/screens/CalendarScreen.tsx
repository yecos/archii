...
import { prioColor, taskStColor } from '@/lib/constants';
...
export default function CalendarScreen() {
  ...
  const calFilterProject = useAppStore(s => s.calFilterProject);
  ...
  const setCalFilterProject = useAppStore(s => s.setCalFilterProject);
  ...
  const getUserName = (uid: string) => {
    ...
    return u ? u.data.name : uid.substring(0, 8) + '...';
  };
  ...
  const prevMonth = () => { if (calMonth === 0) { setCalMonth(11); setCalYear(calYear - 1); } else { setCalMonth(calMonth - 1); } setCalSelectedDate(null); };
  const nextMonth = () => { if (calMonth === 11) { setCalMonth(0); setCalYear(calYear + 1); } else { setCalMonth(calMonth + 1); } setCalSelectedDate(null); };

  // TODO: connect to openEditMeeting (defined in page.tsx)
  ...
  // TODO: connect to deleteMeeting (defined in page.tsx)
