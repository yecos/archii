...
<div className="animate-fadeIn space-y-4">
        ...
        {/* Obra Report — RFIs, Submittals, Punch List */}
        {forms.reportTab === 'Obra' && (<div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {(() => {
            ...
            const punchStatusData = [
              { name: 'Pendiente', value: punchPending },
              { name: 'En progreso', value: punchProgress },
              { name: 'Completado', value: punchDone },
            ].filter(d => d.value > 0);
            ...
            return (<>
              ...
              <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-5">
                ...
                {submittals.length > 0 && <div className="mt-4 text-xs text-[var(--muted-foreground)]">Tasa de aprobación: <span className="font-semibold text-emerald-400">{submittals.length > 0 ? Math.round((subApproved / submittals.length) * 100) : 0}%</span> de {submittals.length} total</div>}
              </div>
